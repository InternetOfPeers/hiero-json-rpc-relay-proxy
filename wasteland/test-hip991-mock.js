...
async function testHIP991Structure() {
  ...
  test(
    'proxyManager',
    'ProxyHederaManager has submitMessageToTopic method',
    () => {
      const manager = new ProxyHederaManager({});
  ...
  test(
    'proverManager',
    'ProverHederaManager has submitMessageToTopic method',
    () => {
      const manager = new ProverHederaManager({});
      if (typeof manager.submitMessageToTopic !== 'function') {
        throw new Error('submitMessageToTopic method not found');
  ...
  test(
    'proverManager',
    'ProverHederaManager has configureTopicForProver method',
    () => {
      const manager = new ProverHederaManager({});
  ...
  test(
    'proverManager',
    'ProverHederaManager has checkTopicExists method',
    () => {
      const manager = new ProverHederaManager({});
      if (typeof manager.checkTopicExists !== 'function') {
        throw new Error('checkTopicExists method not found');
  ...
  test('proxyManager', 'Proxy imports HIP-991 classes', () => {
    ...
    const proxyContent = fs.readFileSync(
      './packages/proxy/src/hederaManager.js',
      'utf8'
    );
  ...
  test('proverManager', 'Prover imports HIP-991 classes', () => {
    const fs = require('fs');
    const proverContent = fs.readFileSync(
      './packages/prover/src/hederaManager.js',
      'utf8'
    );
    ...
    const missingImports = requiredImports.filter(
      imp => !proverContent.includes(imp)
    );
    if (missingImports.length > 0) {
      throw new Error(`Missing imports: ${missingImports.join(', ')}`);
    }
  ...
  const totalTests =
    results.proxyManager.pass +
    results.proxyManager.fail +
    results.proverManager.pass +
    results.proverManager.fail;
  ...
  console.log(
    `📈 Proxy Manager: ${results.proxyManager.pass}/${results.proxyManager.pass + results.proxyManager.fail} tests passed`
  );
  console.log(
    `📈 Prover Manager: ${results.proverManager.pass}/${results.proverManager.pass + results.proverManager.fail} tests passed`
  );
