...
async function testHIP991Implementation() {
    ...
    if (!proxyConfig.accountId || !proxyConfig.privateKey) {
        ...
        console.error('   Please ensure HEDERA_ACCOUNT_ID and HEDERA_PRIVATE_KEY are set');
        process.exit(1);
    }
    ...
    try {
        ...
        console.log('⏳ Waiting 10 seconds for topic to propagate across Hedera network...');
        ...
        const proverReceipt = await proverManager.submitMessageToTopic(topicId, proverMessage);
        console.log(`✅ Prover message submitted - Sequence: ${proverReceipt.topicSequenceNumber}\n`);
        ...
        const proxyReceipt = await proxyManager.submitMessageToTopic(topicId, proxyMessage);
        console.log(`✅ Proxy message submitted - Sequence: ${proxyReceipt.topicSequenceNumber}\n`);
